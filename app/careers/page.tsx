...
import { getCmsPage, getPublishedJobs } from "@/lib/cms";
export const metadata = { title: "Careers" };
export const revalidate = 60;
export default async function Page() {
  const [jobs,page]=await Promise.all([getPublishedJobs(),getCmsPage("careers")]);
  return <>
    <section className="page-hero page-hero-dark"><div className="container narrow"><span className="eyebrow">{page?.eyebrow||"Careers"}</span><h1>{page?.headline||"Build the future of entertainment and media."}</h1><p>{page?.summary||"Join a company creating entertainment and platforms designed to help people get more out of life."}</p></div></section>
    ...
    <section className="section"><div className="container"><div className="section-heading section-heading--row"><div><span className="eyebrow">Open roles</span><h2>{jobs.length?"Find your place at Plekxa.":"There are currently no public openings."}</h2></div><p>New roles are published here as they open.</p></div>{jobs.length?<div className="jobs-grid">{jobs.map(job=><article className="job-card" key={job.id}><div><span>{job.department||"Plekxa"}</span><h3>{job.title}</h3><p>{[job.location,job.employment_type].filter(Boolean).join(" · ")}</p></div>{job.summary&&<p>{job.summary}</p>}{job.application_url?<a href={job.application_url} className="arrow-link" target="_blank" rel="noreferrer">Apply now <b>↗</b></a>:<Link href={`/contact?topic=${encodeURIComponent(`Career: ${job.title}`)}`} className="arrow-link">Enquire <b>↗</b></Link>}</article>)}</div>:<div className="empty-state"><p>New opportunities will appear here when they open.</p></div>}</div></section>
  </>;
}
